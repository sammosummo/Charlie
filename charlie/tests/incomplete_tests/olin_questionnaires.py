...
This "test" administers a set of self-report questionnaires. Unlike the normal
Pygame or GUI-based tests, this script creates a web page containing the
questionnaires, which should be opened automatically in the computer's default
browser. Because there are so many different browsers and operating systems,
this behaviour cannot be guaranteed every time, so the URL of the web page is
displayed in the terminal. This method requires creating a temporary local
web server to process the incoming form information. When the form is
submitted by the proband, the form information is converted and stored within a
regular data object.

The questionnaires are the Beck Depression Inventory version II [1], the
Depression Anxiety and Stress Scale [2], the Barratt Impulsiveness Scale
version 11 [3], the Stat-Trait Anxiety Inventory for Adults [4], and the
Fagerstroem Nicotine Dependence Scale [4]. Individual items
