...
def get_universal_entries(data_obj):
    ...
    :return: (cols, entries) : cols is a list of column names; entries is a
                               list of values
    """
    ...
    lang = data_obj.lang
    cols = [
        'test_name', 'proband_id', 'date_started', 'date_completed',
        'time_taken', 'computer_os', 'computer_name', 'computer_user',
        'user_id', 'proj_id', 'lang'
    ]
    entries = [
        test_name, proband_id, str(initialised)[:19], str(date_done)[:19],
        str(date_done - initialised), system(), gethostname(), getuser(),
        user_id, proj_id, lang
    ]
    return cols, entries


def sdt_dprime(N, S, H, F,):
    """
    Returns d'
